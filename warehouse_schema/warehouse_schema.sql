-- ============================================================
-- DATA WAREHOUSE SCHEMA (Star Schema — Canonical)
-- E-commerce Data Pipeline with Automated ETL
--
-- Proper star schema with clear FACT / DIMENSION separation.
-- Grain: fact_order_items = one row per order line item.
-- ============================================================

-- ────────────────────────────────────────────
-- DIMENSION: Date
-- One row per calendar day. Generated by ETL.
-- ────────────────────────────────────────────
...
-- ────────────────────────────────────────────
-- DIMENSION: Customer
-- Descriptive customer attributes + segment.
-- ────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS dim_customer (
    customer_key    SERIAL PRIMARY KEY,
    customer_id     INT UNIQUE NOT NULL,       -- natural key from OLTP
...
CREATE TABLE IF NOT EXISTS dim_product (
    ...
    product_id      INT UNIQUE NOT NULL,       -- natural key from OLTP
    ...
    is_active       BOOLEAN DEFAULT TRUE,
    profit_margin   DECIMAL(5,2)               -- pct, calculated by ETL
);
...
-- ────────────────────────────────────────────
-- DIMENSION: Payment Method (Conformed)
-- Pure descriptor — no transactional data.
-- ~6 rows: credit_card, debit_card, upi, etc.
-- ────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS dim_payment_method (
    payment_method_key SERIAL PRIMARY KEY,
    payment_method     VARCHAR(50) UNIQUE NOT NULL,
    display_name       VARCHAR(100) NOT NULL,
    method_type        VARCHAR(30)            -- digital / card / cash
);

-- Seed the conformed dimension
...
-- ────────────────────────────────────────────
-- DIMENSION: Location (Conformed)
-- Geographic hierarchy for analytics.
-- ────────────────────────────────────────────
...
-- Contains all measures for analytics.
-- ────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS fact_order_items (
    ...
    location_key        INT,
    -- Order-level attributes (degenerate)
    ...
    line_total          DECIMAL(12,2) NOT NULL,      -- (qty * price) - discount
    -- Order-level allocations (prorated to line item)
    order_discount_alloc  DECIMAL(10,2) DEFAULT 0,
    shipping_cost_alloc   DECIMAL(10,2) DEFAULT 0,
    net_amount            DECIMAL(12,2) NOT NULL,     -- line_total - order_discount + shipping
    -- Cost measures
    cost_amount           DECIMAL(12,2) DEFAULT 0,    -- qty * cost_price
    gross_profit          DECIMAL(12,2) DEFAULT 0     -- net_amount - cost_amount
);
